...
\begin{ejercicio}{Ejercicio 5. Completar el recorrido de \textsc{IMP}/\textsc{IMP++} a \textsc{C} \hfill {\small [Esencial]}}
El recorrido interno será el mismo para el \textsc{IMP} base y para su extensión \textsc{IMP++}. Completa el flujo con las cuatro fases del banco.
...
programa fuente & \blank[3.3cm] & tokens \\[0.3cm]
...
\begin{enumerate}[leftmargin=*]
    \item ¿Qué versión construiremos en notas y clase, y cuál desarrollaremos en las prácticas?

    \ifrespuestas\else
    \blank[14cm]
    \fi

    \item ¿Qué fases serán apoyadas por Lark?
    \ifrespuestas\else\par\noindent\blank[10cm]\par\fi

    \item ¿Qué fases programaremos mediante recorridos en Python?
    \ifrespuestas\else\par\noindent\blank[10cm]\par\fi
...
\begin{enumerate}[leftmargin=*]
    \item En notas y clase construiremos la versión base para \textsc{IMP}; en las prácticas extenderemos ese producto para \textsc{IMP++}.
    \item Lark apoyará el análisis léxico y el análisis sintáctico.
    \item La verificación semántica y la generación de código se implementarán explícitamente en \textsc{Python} mediante recorridos sobre la representación construida.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item El equipo debe especificar las categorías y reglas léxicas; Lark aplica esa especificación para producir tokens. Primero construiremos las reglas de \textsc{IMP} en clase y después agregaremos las de \textsc{IMP++} en la práctica.
\end{enumerate}
\end{respuesta}
\fi
