...
\begin{ruta}{Ruta de lectura (15--20 minutos)}
\textbf{Lectura esencial:} sigue el caso inicial; localiza las definiciones y comparaciones de las secciones 1 y 3; reconoce las responsabilidades y el recorrido panorámico de la sección 2; y cierra con el esquema \textsc{IMP}/\textsc{IMP++} \(\rightarrow\) \textsc{C} y las cuatro ideas de la sección 4.

\textbf{Lectura complementaria:} vuelve después a los matices sobre la frontera entre aplicaciones y software de sistemas, las combinaciones entre traducción e interpretación y las observaciones que amplían el panorama. La conclusión sirve como comprobación final del recorrido esencial.
\end{ruta}
...
En las notas y clases construiremos progresivamente un compilador para el \textsc{IMP} de Winskel. Las prácticas partirán de esa versión funcional y la extenderán para construir \textsc{IMP++}. Ambos recorridos tendrán como lenguaje destino a \textsc{C}. Por ahora basta representar el objetivo general como una transformación:

\begin{center}
\begin{tikzpicture}[node distance=1.5cm, every node/.style={font=\small}]
\node[draw, rounded corners, fill=grannysmithapple!30, align=center, minimum width=3cm] (imp) {programa en \textsc{IMP}\\o \textsc{IMP++}};
...
\textsc{IMP++} no será un compilador separado: conservará las construcciones de \textsc{IMP} y añadirá, de manera acumulativa, las extensiones aprobadas para las prácticas.
...
El curso no pretende revisar todas las áreas de la programación de sistemas con la misma profundidad. La implementación guiada de \textsc{IMP} y su extensión a \textsc{IMP++} formarán un solo producto acumulativo para estudiar, de manera concreta, cómo se especifica, divide, implementa y prueba una herramienta de sistemas.
\end{observacion}
