...
\textbf{Lectura esencial:} sigue el caso inicial desde los caracteres hasta la secuencia de tokens; distingue token, lexema, patrón y atributo mediante sus definiciones y ejemplos; identifica qué recibe y qué produce el analizador léxico; y resuelve el mini-checkpoint con \texttt{total = 25;}.

\textbf{Lectura complementaria:} revisa después los matices sobre espacios y comentarios, las dos formas de observar la comunicación con el analizador sintáctico y la configuración de \textsc{Lark}. Retoma esta última parte cuando construyamos el lexer de \textsc{IMP} y, después, cuando la Práctica 1 lo extienda para \textsc{IMP++}.
\end{ruta}
...
En notas y clase utilizaremos Lark para construir el lexer funcional del \textsc{IMP} base. La Práctica 1 partirá de esa implementación y agregará las categorías aprobadas de \textsc{IMP++}. Lark podrá leer la entrada y producir tokens a partir de las reglas proporcionadas, pero no decidirá por sí mismo cuáles son las categorías ni qué forma tiene cada una. Para observar el lexer de manera independiente podrá utilizarse Lark con \texttt{parser=None}, \texttt{lexer="basic"} y el método \texttt{lex()}.
\end{observacion}
