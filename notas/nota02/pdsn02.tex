...
En las prácticas del curso, Lark apoyará el reconocimiento léxico y sintáctico. Esto no sustituye la definición de las reglas ni la comprensión de las transformaciones. El estudiantado especificará qué debe reconocerse y observará la salida producida. La verificación semántica se implementará explícitamente en Python mediante recorridos sobre la representación construida.
\end{observacion}
...
\node[draw, rounded corners, fill=grannysmithapple!30, minimum width=2.4cm] (imp) {programa \textsc{IMP}};
...
\node[draw, rounded corners, fill=grannysmithapple!30, minimum width=1.55cm, minimum height=0.8cm] (imp) {\textsc{IMP}};
...
El esquema no fija todavía las estructuras de datos ni la sintaxis de \textsc{IMP}. Establece la dirección de las dependencias: cada práctica extenderá la anterior y deberá conservar interfaces que las fases posteriores puedan utilizar.
...
La arquitectura no consiste sólo en enumerar fases. Cada componente necesita una responsabilidad, una entrada y una salida claras; además, varias fases comparten información mediante estructuras como la tabla de símbolos y colaboran en el diagnóstico de errores. En el compilador acumulativo del curso, este flujo llevará de \textsc{IMP} a \textsc{C}. El siguiente tema concentrará la atención en el primer componente del recorrido: el analizador léxico y las unidades que reconoce.
